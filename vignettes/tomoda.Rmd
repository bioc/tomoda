...
heart of zebrafish, obtained from [GSE74652](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE74652). The 
...
Now we create a Tomo object with the raw read count matrix. Genes expressed in 
less than 3 sections are filtered out. You can change this threshold by changing
the parameter `min.section` of function `CreateTomo`. A Tomo object is an 
instance of `r Biocpkg("SummarizedExperiment")`. If you have additional 
information about sections, save them in `colData(object)`, a data frame used to
save meta data describing sections.
...
If you have an existing SummarizedExperiment object, it can also be used for 
input. Just remember that the object must contain at least one of 'count' assay 
and 'normalized' assay.
```
